...
function activate(context) {
    ...
    const updateWebview = (doc) => {
        ...
        while ((match = rootRegex.exec(text)) !== null) {
            ...
            while ((varMatch = variableRegex.exec(body)) !== null) {
                ...
                if (!flatVariableMap.has(selector))
                    flatVariableMap.set(selector, new Map());
                flatVariableMap.get(selector)?.set(varName, varValue);
            }
        }
        const getVarValue = (selector, varName) => flatVariableMap.get(selector)?.get(varName) ??
            flatVariableMap.get(':root')?.get(varName);
        const resolveValue = (value, selector, depth = 0) => {
            if (depth > 10)
                return null;
        ...
        const selectors = Array.from(new Set(Array.from(variableMap.values()).flatMap(map => Array.from(map.keys()))));
    ...
    let disposable = vscode.commands.registerCommand('cssVariableViewer.showVariables', () => {
        ...
        if (!panel) {
            panel = vscode.window.createWebviewPanel('cssVariables', 'Variables CSS dans :root', vscode.ViewColumn.Beside, { enableScripts: true });
            panel.onDidDispose(() => {
                panel = undefined;
            });
        }
        panel.webview.onDidReceiveMessage(message => {
            ...
            if (message.type === 'rotomecacellclick') {
                ...
                if (variableLine !== -1) {
                    ...
                    editor.selection = new vscode.Selection(variableLine, 0, variableLine, lineText.length);
                    editor.revealRange(new vscode.Range(variableLine, 0, variableLine, lineText.length), vscode.TextEditorRevealType.InCenter);
                }
            }
        });
        updateWebview(doc);
    ...
    const higlight = (event) => {
        // const a = 0;
        // if (!panel) vscode.commands.executeCommand('cssVariableViewer.showVariables');
        // if (!panel) return;
        if (!event.textEditor || !event.textEditor.selections.length)
            return;
    ...
    vscode.window.onDidChangeTextEditorSelection(event => {
        ...
        // Vérifie si la sélection est une variable CSS (ex: --my-var)
        vscode.commands.executeCommand('setContext', 'cssVariableViewer.hasCssVar', /--[\w-]+\s?:/.test(lineText));
    });
    context.subscriptions.push(disposable);
    context.subscriptions.push(vscode.commands.registerCommand('cssVariableViewer.focusVariable', () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor)
            return;
